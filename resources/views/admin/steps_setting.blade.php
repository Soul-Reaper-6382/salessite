...
<div class="content-wrapper">
    ...
    <div class="row justify-content-center">
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <form class="forms-sample" action="{{ route('add_step_settings') }}" method="POST" enctype="multipart/form-data">
                        @csrf

                        <div class="form-group">
                            <label for="imageorvideo" class="form-label">Image or Video</label>
                            <input name="imageorvideo" type="file" class="form-control" id="imageorvideo" required>
                        </div>
                        ...
                        <button type="submit" class="btn btn-gradient-primary me-2 mt-3" style="float:right;">Add Step</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="row justify-content-center">
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">All Steps</h4>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $step->id }}</td>
                                    <td>{{ $step->heading }}</td>
                                     <td>{!! \Illuminate\Support\Str::limit($step->text, 50, '...') !!}</td>
                                    <td>
                                        @if (in_array(pathinfo($step->file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif']))
                                            <img src="{{ url($step->file) }}" style="    width: 100px;
    height: 100px;
    border-radius: 0;">
                                    ...
                                    <td>
                                        @if($step->buttons)
                                            @foreach($step->buttons as $button)
                                                <button class="btn" style="background-color: {{ $button['color'] }}; padding: 5px; margin-bottom: 5px;">
                                                    <a href="{{ $button['link'] }}" style="color: white;">{{ $button['text'] }}</a>
                                                </div>
                                            @endforeach
                                        @endif
                                    </td>
