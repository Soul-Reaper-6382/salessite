...
class DashboardController extends Controller
{
   ...
   //  }
    ...
    public function starting_plan_set(Request $request)
    {
        ...
        $newPlan = Plan::where('id', $newPlanId)->select('id', 'name', 'slug', $stripePlanColumn . ' as stripe_plan', 'price', 'description', 'duration', 'plan')
                     ->first();
        $user->plan_id = $newPlanId;
        $user->save();
        $end_date = \Carbon\Carbon::createFromTimestamp($user->trial_ends_at)->format('Y-m-d');
